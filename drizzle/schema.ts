import { pgTable, pgEnum, varchar, foreignKey, numeric, boolean, primaryKey } from "drizzle-orm/pg-core"
...
export const pokemon = pgTable("pokemon", {
	...
	tipo1: varchar("tipo1", { length: 12 }).notNull().references(() => tipo.nome),
...
export const mossa = pgTable("mossa", {
	id: numeric("id").primaryKey().notNull(),
	nome: varchar("nome", { length: 20 }).notNull(),
	tipo: varchar("tipo", { length: 12 }).notNull().references(() => tipo.nome),
...
export const impara = pgTable("impara", {
	pokemon: numeric("pokemon").notNull().references(() => pokemon.id),
	mossa: numeric("mossa").notNull().references(() => mossa.id),
},
(table) => {
	return {
